...
TEST(SHA256, SmallBuffer)
{
    ...
    for (size_t len = 1; len <= 128; ++len)
    {
        ...
        // 软件实现（基准）
        cks_SHA256_Context soft_ctx = cks_sha256_begin();
        cks_impl_sha256_update_soft(&soft_ctx, data.data(), data.size());
        cks_SHA256 soft_result = cks_impl_sha256_end_soft(&soft_ctx);
        ...
        EXPECT_TRUE(cks_sha256_equal(&result, &soft_result)) << "Failed for length: " << len;

#if CKS_ARCH_X86
        // x86 SHA-NI实现
        {
            cks_SHA256_Context shani_ctx = cks_sha256_begin();
            cks_impl_sha256_update_sha(&shani_ctx, data.data(), data.size());
            cks_SHA256 shani_result = cks_impl_sha256_end_sha(&shani_ctx);
            EXPECT_TRUE(cks_sha256_equal(&shani_result, &soft_result)) << "SHA-NI failed for length: " << len;
        }
#endif
        ...
        {
            cks_SHA256_Context arm_ctx = cks_sha256_begin();
            cks_impl_sha256_update_arm(&arm_ctx, data.data(), data.size());
            cks_SHA256 arm_result = cks_impl_sha256_end_arm(&arm_ctx);
            EXPECT_TRUE(cks_sha256_equal(&arm_result, &soft_result)) << "ARM failed for length: " << len;
        }
#endif
    }
}
...
TEST(SHA256, UnalignedMemory)
{
    ...
    for (size_t offset = 1; offset <= 7; ++offset)
    {
        ...
        // 软件实现（基准）
        cks_SHA256_Context soft_ctx = cks_sha256_begin();
        cks_impl_sha256_update_soft(&soft_ctx, unaligned_data, len);
        cks_SHA256 soft_result = cks_impl_sha256_end_soft(&soft_ctx);

        // 通用接口
        cks_SHA256_Context ctx = cks_sha256_begin();
        cks_sha256_update(&ctx, unaligned_data, len);
        cks_SHA256 result = cks_sha256_end(&ctx);

        EXPECT_TRUE(cks_sha256_equal(&result, &soft_result)) << "Failed for offset: " << offset;

#if CKS_ARCH_X86
        {
            cks_SHA256_Context shani_ctx = cks_sha256_begin();
            cks_impl_sha256_update_sha(&shani_ctx, unaligned_data, len);
            cks_SHA256 shani_result = cks_impl_sha256_end_sha(&shani_ctx);
            EXPECT_TRUE(cks_sha256_equal(&shani_result, &soft_result)) << "SHA-NI failed for offset: " << offset;
        }
#endif
        ...
        {
            cks_SHA256_Context arm_ctx = cks_sha256_begin();
            cks_impl_sha256_update_arm(&arm_ctx, unaligned_data, len);
            cks_SHA256 arm_result = cks_impl_sha256_end_arm(&arm_ctx);
            EXPECT_TRUE(cks_sha256_equal(&arm_result, &soft_result)) << "ARM failed for offset: " << offset;
        }
#endif
    }
}
...
TEST(SHA256, SegmentConsistency)
{
    ...
    cks_SHA256_Context soft_ctx = cks_sha256_begin();
    cks_impl_sha256_update_soft(&soft_ctx, data.data(), data.size());
    cks_SHA256 soft_full = cks_impl_sha256_end_soft(&soft_ctx);
    ...
    // 分段计算 - 软件实现
    {
        ...
        while (pos < data.size())
        {
            ...
            cks_impl_sha256_update_soft(&ctx, data.data() + pos, chunk);
            pos += chunk;
        }
        cks_SHA256 result = cks_impl_sha256_end_soft(&ctx);
        EXPECT_TRUE(cks_sha256_equal(&result, &soft_full));
    }
    ...
    // 分段计算 - 通用接口
    {
        ...
        while (pos < data.size())
        {
            ...
            cks_sha256_update(&ctx, data.data() + pos, chunk);
            pos += chunk;
        }
        cks_SHA256 result = cks_sha256_end(&ctx);
        EXPECT_TRUE(cks_sha256_equal(&result, &soft_full));
    ...
    // 分段计算 - SHA-NI
    {
        ...
        while (pos < data.size())
        {
            ...
            cks_impl_sha256_update_sha(&ctx, data.data() + pos, chunk);
            pos += chunk;
        }
        cks_SHA256 result = cks_impl_sha256_end_sha(&ctx);
        EXPECT_TRUE(cks_sha256_equal(&result, &soft_full));
    }
#endif
    ...
    // 分段计算 - ARM
    {
        ...
        while (pos < data.size())
        {
            ...
            cks_impl_sha256_update_arm(&ctx, data.data() + pos, chunk);
            pos += chunk;
        }
        cks_SHA256 result = cks_impl_sha256_end_arm(&ctx);
        EXPECT_TRUE(cks_sha256_equal(&result, &soft_full));
    }
#endif
}
...
TEST(SHA256, LargeBufferStress)
{
    ...
    EXPECT_TRUE(cks_sha256_equal(&result, &soft_result));
...
#if CKS_ARCH_X86
    {
        ...
        cks_impl_sha256_update_sha(&shani_ctx, data.data(), data.size());
        cks_SHA256 shani_result = cks_impl_sha256_end_sha(&shani_ctx);
        EXPECT_TRUE(cks_sha256_equal(&shani_result, &soft_result));
    }
#endif
...
#if CKS_ARCH_ARM
    {
        ...
        cks_impl_sha256_update_arm(&arm_ctx, data.data(), data.size());
        cks_SHA256 arm_result = cks_impl_sha256_end_arm(&arm_ctx);
        EXPECT_TRUE(cks_sha256_equal(&arm_result, &soft_result));
    }
#endif
}
...
TEST(SHA256, MillionA)
{
    ...
    {
        cks_sha256_update(&ctx, chunk, sizeof(chunk));
    }
    ...
    for (size_t i = 0; i < count / sizeof(chunk); ++i)
    {
        cks_impl_sha256_update_soft(&soft_ctx, chunk, sizeof(chunk));
    }
    cks_SHA256 soft_result = cks_impl_sha256_end_soft(&soft_ctx);
    EXPECT_EQ(to_hex(soft_result), expected_hex);
    EXPECT_TRUE(cks_sha256_equal(&result, &soft_result));
